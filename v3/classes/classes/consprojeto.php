...
//ini_set('display_errors','1');	  
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			...
			$sqlexperimento = 'select count(*) from modelr.experiment where idproject = '.$row['idproject'];
			$resexperimento = pg_exec($this->conn,$sqlexperimento);
			$rowexperimento = pg_fetch_array($resexperimento);
			$qtd = $rowexperimento[0];
			
			
			$html = '<td class="a-center "><input type="checkbox" class="flat" name="id_projeto[]" id="id_projeto" value="'.$row["idproject"].'" ></td>
                                    <td class=" ">'.$row['project'].'</td>
                                    <td class=" ">'.$row['description'].'</td>
                                    <td class=" ">'.$row['nome'].'</td>
                                    <td class=" ">'.$qtd.'</td>
                                    <td class=" last">
									<a href="consexperimento.php?idproject='.$row['idproject'].'" class="btn btn-warning btn-xs"><i class="fa fa-gear"></i> Abrir</a>
									<a href="cadprojeto.php?op=A&id='.$row['idproject'].'" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Editar</a>
...
$Paginacao = new MyPag();
$Paginacao->conn = $conn;
$sql = 'select * from modelr.project, modelr.usuario where project.idusuario = usuario.idusuario and project.idproject = idproject
 ';

if ($tipofiltro=='PROJECT')
{
   $sql.= " and project ilike '%".$valorfiltro."%'";	
}
if ($tipofiltro=='DESCRIPTION')
{
   $sql.= " and description ilike '%".$valorfiltro."%'";	
}


if ($_SESSION['s_idtipousuario']!='2')
{
	$sql.=' and usuario.idusuario = '.$_SESSION['s_idusuario'];
}


if (($ordenapor=='PROJECT') || ($ordenapor==''))
{
                ...
                <div class="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title">
                                    <h2>Consulta Projeto <small>Projetos cadastrados no sistema</small></h2>
                                ...
                                <span class="caret"></span>
                                         ...
                                         <ul id="menu6" class="dropdown-menu animated fadeInDown" role="menu">
                                            ...
                                            <li role="presentation"><a role="menuitem" tabindex="-1" href="cadprojeto.php?op=I">Novo</a>
                                            </li>
								...
								<div class="form-group">
                                    ...
                                    <select id="cmboxtipofiltro" name="cmboxtipofiltro" class="form-control">
                                                    <option value="PROJECT" <?php if ($tipofiltro=='PROJECT') echo "selected";?>>Projeto</option>
                                                    <option value="DESCRIPTION" <?php if ($tipofiltro=='DESCRIPTION') echo "selected";?>>Descrição</option>
                                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="cmboxordenar" name="cmboxordenar" class="form-control">
                                                    <option value="PROJECT" <?php if ($ordenapor=='PROJECT') echo "selected";?>>Projeto</option>
                                                    </select>
                                </div>
								<button type="button" class="btn btn-success" onClick='filterApply()'>Filtrar</button>
								<button type="button" class="btn btn-info" onClick='filterApply()'>Novo</button>
								<button type="button" class="btn btn-error" onClick='filterApply()'>Excluir</button>
