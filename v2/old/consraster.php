...
//	  require_once('classes/categoira.class.php');
	...
	class MyPag extends Paginacao
	{
		function desenhacabeca($row)
		{
			 ...
			 <thead>
                                            <tr class="headings">
                                                ...
                                                <th class="column-title">Fonte</th>
                                                <th class="column-title">Raster</th>
                                                <th class="column-title">fonte</th>
		...
		function desenha($row){
			$html = '<td class="a-center "><input type="checkbox" class="flat" name="id_raster[]" id="id_raster" value="'.$row["idraster"].'" ></td>
                                    <td class=" ">'.$row['source'].'</td>
                                    <td class=" ">'.$row['raster'].'</td>
                                    <td class=" ">'.$row['periodo'].'</td>
                                    <td class=" last"><a href="cadraster.php?op=A&id='.$row['idraster'].'" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>
                                                    </td>';
...
$Paginacao = new MyPag();
$Paginacao->conn = $conn;
$sql = 'select * from modelr.raster r, modelr.source s  where r.idsource = s.idsource
 ';

if ($tipofiltro=='RASTER')
{
   $sql.= " and raster ilike '%".$valorfiltro."%'";	
}



if (($ordenapor=='RASTER') || ($ordenapor==''))
{
   $sql.= " order by raster";	
}
                ...
                <div class="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title">
                                    <h2>Consulta Raster <small>Raster cadastrados no sistema</small></h2>
								...
								<div class="form-group">
                                    ...
                                    <select id="cmboxtipofiltro" name="cmboxtipofiltro" class="form-control">
                                                    <option value="RASTER" <?php if ($tipofiltro=='RASTER') echo "selected";?>>Raster</option>
                                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="cmboxordenar" name="cmboxordenar" class="form-control">
                                                    <option value="RASTER" <?php if ($ordenapor=='RASTER') echo "selected";?>>Raster</option>
                                                    </select>
                                </div>
