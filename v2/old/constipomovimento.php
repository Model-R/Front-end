...
function montapaginacao(pag,base,ordenacao)
{
	...
	dojo.xhrGet({
		url: "montapaginacaotipomovimento.php?p="+pag+"&base="+base+"&o="+ordenacao+"&tf=<?php echo $_REQUEST['cmboxfiltro'];?>&vf=<?php echo $_REQUEST['edtfiltro'];?>",
...
<font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Tipo Movimento</strong></font> 
  <div style="width: 100%; height: 100%;">
			<form action="constipomovimento.php" method="POST" name="formulario"  id="formulario">		
        <input type="hidden" name="op" value="E">
		<table>
			   	<tr>
				   <td>Filtro:</td>
				   <td><select name="cmboxfiltro" id="cmboxfiltro">
						 <option value="CODIGO" <?php if ($_REQUEST['cmboxfiltro']=='CODIGO'){ echo "SELECTED"; } ?>>Código</option>
						 <option value="CATEGORIA" <?php if ($_REQUEST['cmboxfiltro']=='CATEGORIA'){ echo "SELECTED"; } ?>>Categoria</option>
						 <option value="TIPO MOVIMENTO" <?php if ($_REQUEST['cmboxfiltro']=='TIPO MOVIMENTO'){ echo "SELECTED"; } ?>>Tipo Movimento</option>
						 </select>&nbsp;
					...
					<td><select name="cmboxordenacao" id="cmboxordenacao" onChange="enviarFormulario();">
						 <option value="CODIGO" <?php if ($_REQUEST['cmboxordenacao']=='CODIGO'){ echo "SELECTED"; } ?>>Código</option>
						 <option value="CATEGORIA" <?php if ($_REQUEST['cmboxordenacao']=='CATEGORIA'){ echo "SELECTED"; } ?>>Categoria</option>
						 <option value="TIPO MOVIMENTO" <?php if ($_REQUEST['cmboxordenacao']=='TIPO MOVIMENTO'){ echo "SELECTED"; } ?>>Tipo Movimento</option>
						 </select>
					</td>
			   	</tr>
			   	</table>
	...
	function checkdata() {
		with(document.formulario) {
			if(id_usuario.checked == false) {
				alert("Nenhuma tipo movimento selecionada!");
				return false;
			}else{
				if (confirm('Deseja realmente excluir tipo movimentos?')){
	...
	function novo()
	{
		location.href='cadtipomovimento.php?op=I';
	}

    function verificaSelecao()
	{
		var chks = document.getElementsByName('id_tipomovimento[]');
		...
		if (c>1)
		{
		   alert('Selecione apenas uma tipo movimento');
		   return 0;	
		}
		else
		{
			if (c<1)
			{
				alert('Selecione uma tipo movimento');
				return 0;
			}
			else
			{
