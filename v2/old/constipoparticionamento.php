...
//	  require_once('classes/categoira.class.php');
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			$html = '<td class="a-center "><input type="checkbox" class="flat" name="id_tipoparticionamento[]" id="id_tipoparticionamento" value="'.$row["idpartitiontype"].'" ></td>
                                    <td class=" ">'.$row['partitiontype'].'</td>
                                    <td class=" last"><a href="cadtipoparticionamento.php?op=A&id='.$row['idpartitiontype'].'" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a>
                                                    </td>';
...
$Paginacao = new MyPag();
$Paginacao->conn = $conn;
$sql = 'select * from modelr.partitiontype where idpartitiontype = idpartitiontype
 ';

if ($tipofiltro=='NOME')
{
   $sql.= " and partitiontype ilike '%".$valorfiltro."%'";	
}
