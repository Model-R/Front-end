...
<?php require_once('classes/conexao.class.php');
	...
	class MyPag extends Paginacao
	{
		function desenhacabeca($row)
		{
			 ...
			 <thead>
                                            <tr class="headings">
                                                ...
                                                <th class="column-title">Mãe</th>
                                                <th class="column-title">Pai </th>
                                                <th class="column-title">Porte </th>
                                                <th class="column-title">Est. Reprod.</th>';
		...
		function desenha($row){
			...
			$acoes .= '<a target="_blank" href="consparto.php?idprodutor='.$idprodutor.'&numero='.$row['numero'].'&ano='.$row['ano'].'&idpropriedade='.$row['idpropriedade'].'" class="btn btn-primary btn-xs"><i class="fa fa-files-o"></i></a>';
			$acoes .= '<a target="_blank" href="cadlactacao.php?idprodutor='.$idprodutor.'&numero='.$row['numero'].'&ano='.$row['ano'].'&idpropriedade='.$row['idpropriedade'].'" class="btn btn-primary btn-xs"><i class="fa fa-trash-o"></i></a>';
			$acoes .= '<a target="_blank" href="conscobertura.php?idprodutor='.$idprodutor.'&numero='.$row['numero'].'&ano='.$row['ano'].'&idpropriedade='.$row['idpropriedade'].'" class="btn btn-primary btn-xs"><i class="fa fa-sign-in"></i></a>';
			$acoes .= '<a target="_blank" href="consbalanca.php?idprodutor='.$idprodutor.'&numero='.$row['numero'].'&ano='.$row['ano'].'&idpropriedade='.$row['idpropriedade'].'" class="btn btn-primary btn-xs"><i class="fa fa-line-chart"></i></a>';
			...
			$html = '<td class="a-center '.$inativo.'"><input type="checkbox" class="flat" name="id_visitatecnica[]" id="id_visitatecnica" value="'.$row["idvisitatecnica"].'" ></td>
                                    ...
                                    <td class="'.$inativo.'">'.$row['numero'].'</td>
                                    <td class="'.$inativo.'">'.$row['categoriaanimal'].'</td>
                                    <td class="'.$inativo.'">'.$row['nome'].'</td>
                                    <td class="'.$inativo.'">'.date('d/m/Y',strtotime($row['datanascimento'])).'</td>
                                    <td class="'.$inativo.'">'.$row['nomemae'].'</td>
                                    <td class="'.$inativo.'">'.$row['nomepai'].'</td>
                                    <td class="'.$inativo.'">'.$row['porte'].'</td>
                                    <td class="'.$inativo.'">'.$row['estadoreprodutivo'].'</td>';
									$html.='
                                    <td class=" last">'.$acoes.'
                                    </td>';
...
$sql="
select * from rebanho r, categoriaanimal ca where
r.idcategoriaanimal = ca.idcategoriaanimal and r.idpropriedade = ".$idpropriedade." and r.ano = ".$ano."
";	
					 ...
					 <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <div class="row">
								...
								<div class="col-md-3 col-sm-12 col-xs-12 form-group">
                                    <select name="cmboxano" id="cmboxano" onChange="submit()" class="form-control">
                            ...
                            <div class="x_panel">
                                <div class="x_title">
                                    <h2>Acompanhamento Zootécnico (Rebanho)<small></small></h2>
                                    <ul class="nav navbar-right panel_toolbox">
                                ...
                                <span class="caret"></span>
                                        ...
                                        <ul id="menu6" class="dropdown-menu animated fadeInDown" role="menu">
                                            <li role="presentation"><a role="menuitem" tabindex="-1" href="cadrebanho.php?op=I&idprodutor=<?php echo $idprodutor;?>&ano=<?php echo $ano;?>&idpropriedade=<?php echo $idpropriedade;?>">Novo</a>
                                            </li>
