...
function montapaginacao(pag,base,ordenacao)
{
	...
	dojo.xhrGet({
		url: "montapaginacaocategoriatipomovimento.php?p="+pag+"&base="+base+"&o="+ordenacao+"&tf=<?php echo $_REQUEST['cmboxfiltro'];?>&vf=<?php echo $_REQUEST['edtfiltro'];?>",
...
<font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Categoria 
  Tipo Movimento</strong></font> 
  <div style="width: 100%; height: 100%;">
			<form action="conscategoriatipomovimento.php" method="POST" name="formulario"  id="formulario">		
	...
	function checkdata() {
		with(document.formulario) {
			if(id_usuario.checked == false) {
				alert("Nenhuma categoria tipo movimento selecionada!");
				return false;
			}else{
				if (confirm('Deseja realmente excluir as categoria tipo movimento?')){
	...
	function novo()
	{
		location.href='cadcategoriatipomovimento.php?op=I';
	}

    function verificaSelecao()
	{
		var chks = document.getElementsByName('id_categoriatipomovimento[]');
		...
		if (c>1)
		{
		   alert('Selecione apenas uma categoria tipo movimento');
		   return 0;	
		}
		else
		{
			if (c<1)
			{
				alert('Selecione uma categoria tipo movimento');
				return 0;
			}
			else
			{
