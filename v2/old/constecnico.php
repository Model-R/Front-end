...
<?php require_once('classes/conexao.class.php');
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			...
			$sql2 = 'select * from usuario where idtecnico = '.$row['idtecnico'];
			$res2 = pg_exec($this->conn,$sql2);
			$row2 = pg_fetch_array($res2);
			
			$html = '<td class="a-center "><input type="checkbox" class="flat" name="id_[]" id="id_" value="'.$row["idtecnico"].'" ></td>
                                    <td class=" ">'.$row['nometecnico'].'</td>
                                    
                                    <td class=" ">'.$row['email'].'</td>
                                    <td class=" ">'.$row['endereco'].'</td>
                                    <td class=" ">'.$row['telefone'].'</td>
                                    <td class="a-right a-right ">'.$row['celular'].'</td>
                                    <td class="">'.$row2['situacao'].'</td>
                                    <td class=" last"><a href="cadtecnico.php?op=A&id='.$row['idtecnico'].'" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a>
                                                    </td>';
...
$Paginacao = new MyPag();
$Paginacao->conn = $conn;
$sql = 'select * from tecnico t where t.idtecnico = t.idtecnico ';
//
//tecnico t inner join propriedade prop on
//prop.idtecnico = t.idtecnico

if ($ativo=='S')
{
	$sql.=" and (t.idtecnico in (select idtecnico from usuario u where u.situacao='ATIVO')) ";
}

if ($tipofiltro=='NOME')
{
   $sql.= " and t.nometecnico ilike '%".$valorfiltro."%'";	
}
if ($tipofiltro=='PROPRIEDADE')
{
   $sql.= " and prop.nomepropriedade ilike '%".$valorfiltro."%'";	
}
...
if ($ordenapor=='EMAIL')
{
   $sql.= " order by upper(sem_acentos(t.email))";	
}
                ...
                <div class="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="x_panel">
                                <div class="x_title">
                                    <h2>Consulta Técnico <small>Técnicos cadastrados no sistema</small></h2>
                                ...
                                <span class="caret"></span>
                                        ...
                                        <ul id="menu6" class="dropdown-menu animated fadeInDown" role="menu">
                                            <li role="presentation"><a role="menuitem" tabindex="-1" href="cadtecnico.php?op=I">Novo</a>
                                            </li>
...
<form class="form-inline" name="frm" id="frm" method="post">
								...
								<div class="form-group">
                                    ...
                                    <select id="cmboxtipofiltro" name="cmboxtipofiltro" class="form-control">
                                                    ...
                                                    <option value="CPF" <?php if ($tipofiltro=='CPF') echo "selected";?>>CPF</option>
                                                    <option value="PROPRIEDADE" <?php if ($tipofiltro=='PROPRIEDADE') echo "selected";?>>Propriedade</option>
                                                </select>
                                </div>
                                <div class="form-group">
								...
								<div class="form-group">
                                    ...
                                    <select id="cmboxordenar" name="cmboxordenar" class="form-control">
                                                    ...
                                                    <option value="EMAIL" <?php if ($ordenapor=='EMAIL') echo "selected";?>>E-mail</option>
                                                </select>
                                </div>
                            </p>
