...
class Usuario
{
	...
	function incluir()
	{
		...
		$resultado = pg_exec($this->conn,$sql);
       	if ($resultado){
			$sql = "select max(idusuario) from modelr.usuario";
...
function enviarSenha($email){
	...
	$sql = "select * from modelr.usuario where email = '".$email."'";
	$res = pg_exec($this->conn,$sql);
...
//	if (resultado)
	...
	function trocarSenha($senha,$novasenha,$confirmacaosenha)
	{
		...
		$sql = " select * from modelr.usuario where idusuario = '".$this->idusuario."'";

		$result = pg_exec($this->conn,$sql);

		if ((pg_num_rows($result)>0) && ($r = true)  ){
			$sql = " update modelr.usuario set senha = '".$novasenha."' where idusuario = '".$this->idusuario."'";
