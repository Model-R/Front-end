...
<?php 
	...
	class MyPag extends Paginacao
	{
		...
		function desenhacabeca($row)
		{
		 	 $html = '
                     <tr valign="top" class="tab_bg_2"> 
                      <th width="3%">&nbsp;</th>
                      <th width="20%">Tipo de Capital</th>
                      </tr>
                      ';
		 		echo $html;
		}
		...
		function desenha($row){
			 ...
			 $html = ' 
                      <td align="center"><input type="radio" name="id_tipocapital" id="id_tipocapital" value="'.$row["idtipocapital"].'" /></td>
					  <td><a href="cadtipocapital.php?op=A&id='.$row["idtipocapital"].'">'.$row["tipocapital"].'</a></td>
 					  ';
		 		echo $html;
				echo "";
		}// function
	...
	$paginacao = new MyPag();
	$paginacao->conn = $conn;
    $sql = ' select * from tipocapital where
	idtipocapital = idtipocapital';
	// fim ajuste no sistema
	$o = $_REQUEST['o'];
	$tipofiltro = $_REQUEST['tf'];
	$valorfiltro = $_REQUEST['vf'];

	if ($tipofiltro == 'TIPOCAPITAL')
	{
		$sql_where = ' and upper(tipocapital) ilike upper(\'%'.$valorfiltro.'%\')';
	}
	...
	if ($o == 'TIPOCAPITAL')
	{
	   $sql_ordenacao = ' order by upper(tipocapital)';
	}
