...
<?php 
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			 ...
			 $html = ' 
                      <td align="center"><input type="radio" name="id_avaliacao" id="id_avaliacao" value="'.$row["idavaliacao"].'" /></td>
					  <td><a href="'.$url.'.php?op=A&id='.$row["idavaliacao"].'">'.$row["anoreferencia"].'</a></td>
 					  <td><a href="'.$url.'.php?op=A&id='.$row["idavaliacao"].'">'.$row["avaliacao"].'</a></td>
