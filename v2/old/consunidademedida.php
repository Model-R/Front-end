...
function montapaginacao(pag,base,ordenacao)
{
	...
	dojo.xhrGet({
		url: "montapaginacaounidademedida.php?p="+pag+"&base="+base+"&o="+ordenacao+"&tf=<?php echo $_REQUEST['cmboxfiltro'];?>&vf=<?php echo $_REQUEST['edtfiltro'];?>",
...
<font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Unidade Medida</strong></font> 
  <div style="width: 100%; height: 100%;">
			<form action="consunidademedida.php" method="POST" name="formulario"  id="formulario">		
        <input type="hidden" name="op" value="E">
		<table>
			   	<tr>
				   <td>Filtro:</td>
				   <td><select name="cmboxfiltro" id="cmboxfiltro">
						 <option value="UNIDADE MEDIDA" <?php if ($_REQUEST['cmboxfiltro']=='UNIDADE MEDIDA'){ echo "SELECTED"; } ?>>Unidade Medida</option>
						 <option value="SIGLA" <?php if ($_REQUEST['cmboxfiltro']=='SIGLA'){ echo "SELECTED"; } ?>>Sigla</option>
						 </select>&nbsp;
					...
					<td><select name="cmboxordenacao" id="cmboxordenacao" onChange="enviarFormulario();">
						 <option value="UNIDADE MEDIDA" <?php if ($_REQUEST['cmboxordenacao']=='UNIDADE MEDIDA'){ echo "SELECTED"; } ?>>Unidade Medida</option>
						 <option value="SIGLA" <?php if ($_REQUEST['cmboxordenacao']=='SIGLA'){ echo "SELECTED"; } ?>>Sigla</option>
						 </select>
					</td>
			   	</tr>
			   	</table>
	...
	function checkdata() {
		with(document.formulario) {
			if(id_usuario.checked == false) {
				alert("Nenhuma unidade de medida selecionada!");
				return false;
			}else{
				if (confirm('Deseja realmente excluir as unidade de medidas selecionadas?')){
	...
	function novo()
	{
		location.href='cadunidademedida.php?op=I';
	}

    function verificaSelecao()
	{
		var chks = document.getElementsByName('id_unidademedida[]');
		...
		if (c>1)
		{
		   alert('Selecione apenas uma unidade de medida');
		   return 0;	
		}
		else
		{
