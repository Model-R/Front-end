...
function montapaginacao(pag,base,ordenacao)
{
	...
	dojo.xhrGet({
		url: "montapaginacaounidade.php?p="+pag+"&base="+base+"&o="+ordenacao+"&tf=<?php echo $_REQUEST['cmboxfiltro'];?>&vf=<?php echo $_REQUEST['edtfiltro'];?>",
...
<font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Unidade Medida</strong></font> 
  <div style="width: 100%; height: 100%;">
			<form action="consunidade.php" method="POST" name="formulario"  id="formulario">		
        <input type="hidden" name="op" value="E">
		<table>
			   	<tr>
				   <td>Filtro:</td>
				   <td><select name="cmboxfiltro" id="cmboxfiltro">
						 <option value="UNIDADE" <?php if ($_REQUEST['cmboxfiltro']=='UNIDADE'){ echo "SELECTED"; } ?>>Unidade</option>
						 </select>&nbsp;
					...
					<td><select name="cmboxordenacao" id="cmboxordenacao" onChange="enviarFormulario();">
						 <option value="UNIDADE" <?php if ($_REQUEST['cmboxordenacao']=='UNIDADE'){ echo "SELECTED"; } ?>>Unidade</option>
						 </select>
					</td>
			   	</tr>
			   	</table>
	...
	function checkdata() {
		with(document.formulario) {
			if(id_usuario.checked == false) {
				...
				if (confirm('Deseja realmente excluir as unidade selecionadas?')){
	...
	function novo()
	{
		location.href='cadunidade.php?op=I';
	}

    function verificaSelecao()
	{
		var chks = document.getElementsByName('id_unidade[]');
		...
		if (c>1)
		{
		   alert('Selecione apenas uma unidade');
		   return 0;	
		}
		else
		{
