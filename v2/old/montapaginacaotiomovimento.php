...
<?php 
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			 ...
			 $html = ' 
                      <td align="center"><input type="radio" name="id_tipomovimento" id="id_tipomovimento" value="'.$row["tipomovimento"].'" /></td>
					  <td><a href="cadtipomovimento.php?op=A&id='.$row["idtipomovimento"].'">'.$row["codigo"].'</a></td>
	...
	$paginacao = new MyPag();
	$paginacao->conn = $conn;
    $sql = ' select * from tipomovimento tm left join unidade u on tm.idunidade = u.idunidade, categoriatipomovimento ctm where
